...
\newcommand{\DataS}{\Data_{\Subsetdom}}
...
\begin{eqnarray}
  ...
  \label{eq:globaldist}
\end{eqnarray}
where $\Subsetdom$ is a finite subset of $\Domain$.  Sometimes, for
...
represented as $(\mu\ x) = \T$ or $(\mu\ x) = \F$, and the usage of
...
\Bernoulli) = \prod_{i\in \Subsetdom} \nu(\Datai | M \in
...
Bernoulli processes with parameter \mintinline{scheme}{p} ranging over
$[0, x]$.  To justify that claim let us first focus on $\nu(M \in
L_{\Bernoullizx} | \DataS, M \in \Bernoulli)$.
\newpage
Thanks to Equation~\ref{eq:nuberiid} we can establish that
...
\begin{eqnarray}
  ...
  \frac{\displaystyle
        \nu(M \in L_{\Bernoullizx} | M \in \Bernoulli)
        \times
        \int_0^x \prod_{i\in \Subsetdom} \nu(\Datai, M \in L_{\Bernoullidt})}
       {\displaystyle
        \int_0^x \nu(M \in L_{\Bernoullidt})
        \times
        \prod_{i\in \Subsetdom} \nu(\Datai | M \in \Bernoulli)}\\
  & = &
  \frac{\displaystyle
        \nu(M \in L_{\Bernoullizx} | M \in \Bernoulli)
        \times
        \int_0^x \prod_{i\in \Subsetdom} \nu(\Datai | M \in L_{\Bernoulli_{d\theta}})
        \times \nu(M \in L_{\Bernoulli_{d\theta}})}
       {\displaystyle
        \int_0^x \nu(M \in L_{\Bernoullidt})
        \times
        \prod_{i\in \Subsetdom}
