...
probability distribution, $\CDF{\TV{\mu}}$, is defined by the
following equation
\begin{eqnarray}
  \CDF{\TV{\mu}}(x) & = & \nu(M \in L_{\Bernoullizx} | M \in \Bernoulli,
  \DataS) \times \nu(\DataS | M \in \Bernoulli)
\end{eqnarray}
where $L_{\Bernoullizx} \in \Field_{\Bernoulli}$ is the set of
Bernoulli processes with parameter $\theta$ ranging over $[0, x]$.  To
justify that claim let us first focus on $\nu(M \in L_{\Bernoullizx} |
\DataS, M \in \Bernoulli)$.
\newpage
...
\begin{eqnarray}
  ...
  \frac{\displaystyle
       ...
       \label{eq:nuberbinom}
\end{eqnarray}
Equation~\ref{eq:nuberiidcdf} is a special case of
Equation~\ref{eq:nuberiid} where $L_{\Bernoulli}$ is restricted to the
class of Bernoulli process where the parameter $\theta$ ranges from 0
to some value $x \in [0, 1]$, in order to define a CDF.
Equation~\ref{eq:nuberiidsimp} is a simplification of
Equation~\ref{eq:nuberiidcdf} obtained from the fact that
\begin{eqnarray}
  \nu(\Datai | M \in L_{\Bernoullizx})
  %% & = &
  %% \nu(\Datai | M \in L_{\Bernoullizx}, M \in \Bernoulli)\\
...
parameter $\theta$, that is using the additive law of probability,
where $L_{\Bernoullidt}$ is the set of Bernoulli processes with
parameters within an arbitrarily small interval centered around
$\theta$.  Equation~\ref{eq:nuberintcond} is obtained from the
definition of conditional probability.  Equation~\ref{eq:nuberbinom}
is obtained from the likelihood of $(\Datai)_{i\in \Subdom}$ given a
Bernoulli process of parameter $\theta$, where $n$ is the number of
positive pieces of evidence, that is the carnality of $\{i\in \Subdom
| (\mu\ i) = \True\}$, and $N$ is the total number of pieces of
evidence, that is the carnality of $\Subdom$.
