...
\newcommand{\Bernoullip}{\mathcal{B}_p}
...
\end{eqnarray}
where $L_{\Bernoulli[0,x]} \in \Field_{\Bernoulli}$ is the set of
Bernoulli processes with parameter \mintinline{scheme}{p} ranging over
$[0, x]$.  To justify that claim let us first focus on $\nu(M \in
L_{\Bernoulli[0,x]} | \DataS, M \in \Bernoulli)$.  Thanks to
Equation~\ref{eq:nuberiid} we can establish that\\
\begin{minipage}{\textwidth}
  $$\nu(M \in L_{\Bernoulli[0,x]} | \DataS, M \in \Bernoulli)$$
  \begin{eqnarray}
    & = &
    \frac{\prod_{i\in \Subsetdom} \nu(\Datai | M \in L_{\Bernoulli[0,x]})
      \times \nu(M \in L_{\Bernoulli[0,x]} | M \in \Bernoulli)}
         {\prod_{i\in \Subsetdom} \nu(\Datai | M \in \Bernoulli)}
