...
this posterior we need to assume that $\Language$ has a Bernouilli
sampler in its set of operations.  Let $\Bernouilli$ be the subset of
programs of $\Language$ consisting of a single Bernouilli call, thus
$\Bernouilli$ is comprised of the following programs
\begin{minted}{scheme}
  (Bernouilli p)
\end{minted}
...
We can assume that $\Field_{\Bernouilli}$ is a subfield of
$\Field_{\Language}$.  Let us first derive a set of generic equations
involving the event $L_{\Bernouilli} \in \Field_{\Bernouilli}$ knowing
observations $\DataS$ and the fact that the probabilistic programs
under consideration are limited to Bernouilli samplers, that is $M \in
\Bernouilli$:
\begin{eqnarray}
  \nu(M \in L_{\Bernouilli} | M \in \Bernouilli, \DataS) & = &
  \frac{\nu(M \in L_{\Bernouilli}, M \in \Bernouilli, \DataS)}
       {\nu(M \in \Bernouilli, \DataS)} \label{eq:nubercond}\\
  & = &
  \frac{\nu(\DataS | M \in L_{\Bernouilli}, M \in \Bernouilli)
        \times \nu(M \in L_{\Bernouilli}, M \in \Bernouilli)}
       {\nu(M \in \Bernouilli, \DataS)} \label{eq:nubercondnum}\\
  & = &
  \frac{\nu(\DataS | M \in L_{\Bernouilli})
        \times \nu(M \in L_{\Bernouilli}, M \in \Bernouilli)}
       {\nu(M \in \Bernouilli, \DataS)} \label{eq:nubermarkov}\\
  & = &
  \frac{\nu(\DataS | M \in L_{\Bernouilli})
        \times \nu(M \in L_{\Bernouilli} | M \in \Bernouilli)
        \times \nu(M \in \Bernouilli)}
       {\nu(\DataS | M \in \Bernouilli) \times \nu(M \in \Bernouilli)}
       \label{eq:nubercondsubnum}\\
  & = &
  \frac{\nu(\DataS | M \in L_{\Bernouilli})
        \times \nu(M \in L_{\Bernouilli} | M \in \Bernouilli)}
       {\nu(\DataS | M \in \Bernouilli)} \label{eq:nubersmplf}\\
  & = &
  \frac{\prod_{i\in \Subsetdom} \nu(\Datai | M \in L_{\Bernouilli})
        \times \nu(M \in L_{\Bernouilli} | M \in \Bernouilli)}
       {\prod_{i\in \Subsetdom} \nu(\Datai | M \in \Bernouilli)}
       \label{eq:nuberiid}
\end{eqnarray}
Let us walk through each equation one by one.
Equations~\ref{eq:nubercond} and~\ref{eq:nubercondnum} are derived
from the definition of conditional probability.
Equation~\ref{eq:nubermarkov} is obtained from the fact that
$L_{\Bernouilli} \subseteq \Bernouilli$.
...
canceling out $\nu(M \in \Bernouilli)$ in the numerator and
denominator.  If $M \in \Bernouilli$ then observations are independent
and identically distributed (iid), therefore $\nu(\DataS | M \in
\Bernouilli) = \prod_{i\in \Subsetdom} \nu(\Datai | M \in
L_{\Bernouilli})$.  The same reasoning applies to $\nu(\DataS | M \in
L_{\Bernouilli})$ since $L_{\Bernouilli} \subseteq \Bernouilli$,
resulting in Equation~\ref{eq:nuberiid}.

We claim that that if $\mu$ is restricted to the programs in
$\Bernouilli$ then the posterior of \mintinline{scheme}{p} corresponds
to the truth value of $\mu$, expressed as follows NEXT
